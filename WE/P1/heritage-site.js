document.addEventListener('DOMContentLoaded', function () {
    ...
    const siteIndex = urlParams.get('site');
    ...
    if (siteIndex && siteIndex >= 1 && siteIndex <= heritageSites.length) {
        siteImage.src = heritageSites[siteIndex - 1].imageUrl;
    }

    const siteName = document.getElementById('site-name');
    if (siteIndex && siteIndex >= 1 && siteIndex <= heritageSites.length) {
        siteName.textContent = heritageSites[siteIndex - 1].name;
    } else {
        siteName.textContent = 'Heritage Site.';
    }

    const siteDescription = document.getElementById('site-description');
    if (siteIndex && siteIndex >= 1 && siteIndex <= heritageSites.length) {
        siteDescription.textContent = heritageSites[siteIndex - 1].description;
    } else {
        siteDescription.textContent = 'Invalid heritage site.';
    }

    const siteHistory = document.getElementById('site-history');
    if (siteIndex && siteIndex >= 1 && siteIndex <= heritageSites.length) {
        siteHistory.textContent = heritageSites[siteIndex - 1].history;
    } else {
        siteHistory.textContent = 'Invalid heritage site.';
    }
});
